...
function initProductCard(productBox, texturePaths) {
    ...
    let scrollSpeed = 0;
    const scrollData = { scrollFactor: 0 };
    ...
    const tick = () => {
        const elapsedTime = clock.getElapsedTime()/2 + scrollData.scrollFactor;

        camera.position.x = 0.3 * Math.cos(elapsedTime);
        camera.position.y = 0.1 * Math.cos(elapsedTime) + scrollData.scrollFactor; // Увеличиваем коэффициент для более заметного эффекта
